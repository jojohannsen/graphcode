...
def generate_graph_code(graph_name, graph_spec):
    ...
    # Get LLM configuration for code generation (using 'code' section like mk_graph_code.py)
    ...
    try:
        ...
        print(f"Got graph_code_prompt: {graph_code_prompt is not None}", flush=True)
        
        if graph_code_prompt and graph_notation:
            # Note: mk_graph_code.py has a bug with [config['code']['llm_model']] - fixing it here
            prompt = graph_code_prompt.format(
    ...
    try:
        ...
        if isinstance(agent, OpenRouterAgent):
            ...
            # Write to file
            graph_code_file.parent.mkdir(parents=True, exist_ok=True)
            ...
            if graph_code_file.exists():
                with open(graph_code_file, "r") as f:
                    content = f.read()
                print("Agno agent content read from file", flush=True)
            else:
                print("ERROR: Graph code file was not created by Agno agent", flush=True)
                raise ValueError("Graph code file was not created")
        
        # Verify file exists and is not empty
        if not graph_code_file.exists():
            print("ERROR: graph_code.py does not exist", flush=True)
            raise ValueError("graph_code.py was not created")
        
        if graph_code_file.stat().st_size == 0:
            print("ERROR: graph_code.py is empty", flush=True)
            raise ValueError("graph_code.py is empty")
